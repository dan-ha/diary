...
export async function create(username, password, provider, familyName, givenName, middleName, emails, photos) {
    ...
    return SQUser.create({
        username, password, provider, familyName, givenName, middleName, emails: JSON.stringify(emails), photos: JSON.stringify(photos)
    });
}
...
export async function update(username, password, provider, familyName, givenName, middleName, emails, photos) {
    ...
    return user ? user.updateAttributes({
        password, provider, familyName, givenName, middleName, emails: JSON.stringify(emails), photos: JSON.stringify(photos)
    }) : undefined;
}
...
// creates annonymous object with only fields we want to expose
export function sanitizedUser(user) {
    ...
    try {
        res.emails = JSON.parse(user.emails);
    } catch (e) {
        res.emails = [];
    }
    try {
        res.photos = JSON.parse(user.photos);
    } catch (e) {
        res.photos = [];
    }
    return res;
...
export async function findOrCreate(profile) {
    ...
    if (user) return user;
    return await create(profile.id, profile.password, profile.provider, profile.familyName,
        profile.givenName, profile.middleName, profile.emails, profile.photos);
}
